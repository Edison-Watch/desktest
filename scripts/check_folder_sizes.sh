...
EXCLUDE_PATH_RE='(^|/)(target|node_modules|\.venv|venv|\.git|dist|build)(/|$)'
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ] && { [ "$errors" -gt 0 ] || [ "$warnings" -gt 0 ]; }; then
  {
    ...
    echo ""
    echo "**Thresholds:** warn at ${WARN_THRESHOLD} files, error at ${ERROR_THRESHOLD} files. Counts immediate \`.rs\` children only - subfolders are the fix, not the problem."
  } >> "$GITHUB_STEP_SUMMARY"
fi
